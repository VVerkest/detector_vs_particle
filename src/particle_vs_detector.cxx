...
//  Veronica Verkest        August 2, 2017
...
int main () {
  ...
  string outFileName = "out/particle_vs_detector_analysis.root";
  ...
  TH2D* pTMatrix = new TH2D("pTMatrix","Particle p_{T} vs. Detector p_{T};Detector p_{T} (GeV);Particle p_{T} (GeV)",200,0,100,200,0,100);   //  SET TITLES!
  ...
  while ( detReader.NextEvent() ) {       //  BEGIN EVENT LOOP!    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    ClusterSequence particleCluster(part_rawJets, jet_def);    //  CLUSTER EFFICIENCY JETS WITHIN |ETA|<1
    vector<PseudoJet> partRawJets = sorted_by_pt( etaPtSelector(particleCluster.inclusive_jets()) );     // EXTRACT EFFICIENCY JETS
    ...
    for (int j=0; j<detRawJets.size(); j++) {

      if ( partRawJets.size() == 0 ) { continue; }

      jetCircleMatcher.set_reference( detRawJets[j] );
      jetCircleMatcher.sift( partRawJets, localJets, unmatchedJets );

      if ( localJets.size() == 0 ) { continue; }

      matchedJets = sorted_by_pt( localJets );

      detectorJet.push_back( detRawJets[j] );
      particleJet.push_back( matchedJets[0] );
      n_detJets ++;    n_partJets ++;      nMatched ++;

      matchedJets.erase( matchedJets.begin() );
      partRawJets.clear();
      for ( int k=0; k<unmatchedJets.size(); k++ ) { partRawJets.push_back( unmatchedJets[k] ); }
      for ( int k=0; k<matchedJets.size(); k++ ) { partRawJets.push_back( matchedJets[k] ); }
    ...
    for ( int j=0; j<nMatched; j++ ) {
      ...
      trees->pt_diff = ( particleJet[j].pt() - detectorJet[j].pt() );
      trees->eta_diff = ( particleJet[j].eta() - detectorJet[j].eta() );
      trees->phi_diff = ( particleJet[j].phi_std() - detectorJet[j].phi_std() );
      trees->e_diff = ( particleJet[j].e() - detectorJet[j].e() );
      trees->nCons_diff = ( p_Cons.size() - g_Cons.size() );
      trees->delta_R = deltaR;

      pTMatrix->Fill( detectorJet[j].pt(), particleJet[j].pt() );
